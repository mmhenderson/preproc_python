...
def adjust_sess2anat_reg(subject, debug=False):
    ...
    # this transformation MUST be the same for all sessions
    ...
    def change_matrix(m):
        ...
        mnew = mnew[:,[0,2,1,3]]
        mnew *= np.array([1,-1,1,1])

        # this part may need to be edited someday
        # [R to L, P to A, I to S]
        t = [-25, -190, -25]
    ...
    for ss in sess_nums:
        ...
        if os.path.exists(xfm_file_new):
            ...
            xfm_orig = copy.deepcopy(file_utils.read_xfm_mat(autoreg_file))
            
            print(xfm_orig.round())
    
            xfm_adj = change_matrix(xfm_orig)
            
            print(xfm_adj.round())
    
            print('Writing to %s'%(xfm_file_new))
            file_utils.write_xfm_mat(xfm_adj, xfm_file_new)
...
def do_registration(subject, debug=False):
    ...
    for ss in sess_nums:
        ...
        ref_file = os.path.join(preproc_folder, 'RegTemplateSess%02d.nii.gz'%ss)
